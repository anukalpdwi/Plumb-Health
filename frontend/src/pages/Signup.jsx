...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!formData.name || !formData.email || !formData.password || !formData.confirmPassword) {
        setError('Please fill in all clinical profile fields')
        setLoading(false)
        return
      }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center py-12 px-6 lg:px-8 bg-[#fafbfc] font-sans selection:bg-blue-600/20 relative overflow-hidden">
      ...
      >
        <div className="bg-white/90 backdrop-blur-xl py-10 px-8 clinical-shadow rounded-[32px] border border-white relative overflow-hidden group">
          {/* Decorative Corner */}
          ...
          <div className="mt-8 pt-8 border-t border-gray-100 text-center relative z-10">
            <span className="text-gray-500 text-[10px] font-medium uppercase tracking-widest font-['Outfit']">Existing protocol? </span>
            <Link to="/login" className="text-blue-600 hover:text-blue-700 font-bold text-[10px] uppercase tracking-widest border-b border-blue-200 hover:border-blue-400 pb-0.5 ml-2 transition-colors font-['Outfit']">
              Authenticate
            </Link>
          </div>
        </div>
